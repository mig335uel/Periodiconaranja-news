...
// Definimos la interfaz basada en tus datos actuales
interface Post {
  id: string;
  title: string;
  slug: string;
...
export default function HeroSlider({ posts }: { posts: Post[] }) {
  ...
  return (
    <div className="w-full h-[400px] md:h-[500px] lg:h-[600px] relative group rounded-lg overflow-hidden shadow-2xl border-4 border-gray-200 mb-8">
      ...
      >
        {posts.map((post) => (
          <SwiperSlide key={post.id} className="relative w-full h-full bg-gray-900">
            {/* Imagen de fondo */}
            ...
            <div className="absolute bottom-0 left-0 w-full p-6 md:p-12 text-white z-10">
                <div className="max-w-4xl">
                    ...
                    <h2 className="text-2xl md:text-4xl lg:text-5xl font-bold mb-3 leading-tight drop-shadow-lg">
                        <Link href={`/noticias/${post.slug}`} className="hover:text-orange-400 transition-colors">
                            {post.title}
                        </Link>
                    </h2>
                    <p className="hidden md:block text-gray-200 text-sm md:text-lg max-w-2xl drop-shadow-md">
                        ...
                        {post.excerpt ? getExcerpt(post.excerpt, 200) : getExcerpt(post.content, 200)}
                    </p>
                </div>
            </div>
          </SwiperSlide>
