  ...
  useEffect(() => {
    const fetchPostCount = async () => {
      try {
        ...
        if (response.ok) {
          ...
          console.log(data);
          setCountPost(typeof data.count === "number" ? data.count : 0);
        }
      } catch (error) {
    ...
    const fetchCommentsCount = async () => {
      try {
        ...
        if (response.ok) {
          const data = await response.json();
          setCoutComments(typeof data.count === "number" ? data.count : 0);
        }
      } catch (error) {}
    };
    fetchPostCount();
    ...
    <div className="space-y-8 w-full px-8 py-6 bg-linear-to-br from-gray-50 via-white to-gray-100 min-h-screen">
      ...
      <section className="bg-gray-900 rounded-2xl border border-gray-800 p-8 mt-8 shadow-lg">
        ...
        <p className="text-gray-400">
          Aquí podrías colocar un gráfico o análisis de tráfico.
        </p>
      </section>
    </div>
  );
...
function Card({
  ...
  return (
    <div
      className={`flex items-center gap-4 p-5 rounded-2xl ${color} text-white shadow-lg ${shadow} hover:scale-[1.03] transition-transform duration-200`}
    >
      <div className="p-3 bg-white/30 rounded-lg flex items-center justify-center">
        {icon}
      </div>
      <div>
        <p className="text-base font-semibold text-white/90 mb-1 tracking-wide">
          {title}
        </p>
