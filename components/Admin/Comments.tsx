...
// La estructura refleja '*, users(*), posts(title)'
interface Comment {
    id: string;
    content: string;
    status: 'pending' | 'published' | 'spam' | 'deleted';
    created_at: string;
    // La relación incrustada 'users'
    users: Author;
    // La relación incrustada 'posts'
    posts: {
        title: string;
    } | null;
}
...
export default function CommentsAdmin() {
    const [comments, setComments] = useState<ComentariosAdminModule[]>([]);
    ...
    const canModerate = currentUser?.role === 'admin' || currentUser?.role === 'editor';
    ...
    // --- 4. RENDERIZADO DE LA TABLA ---
        const ApproveComment = useCallback(async (comment: ComentariosAdminModule)=>{
        const bodyData = {
            id: comment.id,
            status: 'approved'
        }
                ...
                <table className="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                        {comments.map((comment) => (
                            <tr key={comment.id} className={comment.status === 'spam' ? 'bg-red-50' : ''}>
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 max-w-xs truncate">{comment.content}</td>
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                    {(comment.users?.name + ' ' + comment.users?.last_name) || 'Anónimo'}
                                </td>
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                    {comment.posts?.title || 'N/A'}
                                </td>
                                ...
                                <td className="px-6 py-4 text-right whitespace-nowrap text-sm font-medium">
                                    ...
                                    {canModerate && (
                                        <>
                                            ...
                                            <button className="text-green-600 hover:text-green-900" onClick={() =>ApproveComment(comment)}>
